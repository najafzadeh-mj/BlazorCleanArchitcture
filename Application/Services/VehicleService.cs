    ...
    public class VehicleService(HttpClientService httpClientService) : IVehicleService
    {
        ...
        private static string CheckResponseStatus(HttpResponseMessage response)
        {
            if (!response.IsSuccessStatusCode)
                return $"Sorry unknown error occured.{Environment.NewLine}Error Description{Environment.NewLine}Status Code: {response.StatusCode}";
            else
                return null!;
        }
        ...
        public async Task<GeneralResponse> AddVehicle(CreateVehicleRequestDTO model)
        {
            ...
            if(!string.IsNullOrEmpty(CheckResponseStatus(result))) 
                return ErrorOperation(CheckResponseStatus(result));
        ...
        => await (await PrivateClient())
            .GetFromJsonAsync<IEnumerable<GetVehicleBrandResponseDTO>>(Constant.GetVehicleBrandsRoute);


        
        public async Task<IEnumerable<GetVehicleOwnerResponseDTO>> GetVehicleOwners()
         => await (await PrivateClient())
            .GetFromJsonAsync<IEnumerable<GetVehicleOwnerResponseDTO>>(Constant.GetVehicleOwnersRoute);


        public async Task<IEnumerable<GetVehicleResponseDTO>> GetVehicles()
        => await (await PrivateClient())
            .GetFromJsonAsync<IEnumerable<GetVehicleResponseDTO>>(Constant.GetVehiclesRoute);
